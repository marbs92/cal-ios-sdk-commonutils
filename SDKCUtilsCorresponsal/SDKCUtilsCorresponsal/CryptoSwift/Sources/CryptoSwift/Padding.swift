...
public enum CryptoSwiftPadding: PaddingProtocol {
  ...
  public func add(to: Array<UInt8>, blockSize: Int) -> Array<UInt8> {
    switch self {
      ...
      case .pkcs7:
        return PKCS7.CryptoSwiftPadding().add(to: to, blockSize: blockSize)
      case .pkcs5:
        return PKCS5.CryptoSwiftPadding().add(to: to, blockSize: blockSize)
      case .iso78164:
  ...
  public func remove(from: Array<UInt8>, blockSize: Int?) -> Array<UInt8> {
    switch self {
      ...
      case .pkcs7:
        return PKCS7.CryptoSwiftPadding().remove(from: from, blockSize: blockSize)
      case .pkcs5:
        return PKCS5.CryptoSwiftPadding().remove(from: from, blockSize: blockSize)
      case .iso78164:
